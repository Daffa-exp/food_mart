...
import { Star, MessageSquareText, ShieldCheck, X } from "lucide-react";
...
export default function ProductReviewsBox({ product }: { product: Product }) {
  ...
  return (
    <div className="rounded-card border border-surface-border bg-white p-5">
                  ...
                  <div className="mt-2.5 flex flex-wrap gap-2">
                    {review.photos.map((url, i) => (
                      <button
                        ...
                        className="h-16 w-16 shrink-0 overflow-hidden rounded-input border border-surface-border transition-opacity hover:opacity-90"
                      >
      ...
      <PhotoLightbox url={lightboxPhoto} onClose={() => setLightboxPhoto(null)} />
    </div>
  );
}
